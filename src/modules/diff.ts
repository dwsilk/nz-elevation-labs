...
const diffHandler: AddProtocolAction = async (req, abort) => {
  ...
  const [demImg, dsmImg] = await Promise.all([createImageBitmap(demBlob), createImageBitmap(dsmBlob)]);
...
export function buildDiffColorExpr(): unknown[] {
  return [
    ...
    -10000, 'rgba(0,0,0,0)',
    0,      'rgba(0,0,0,0)',
    1,      'rgba(120,200,100,0.40)',
    5,      'rgba(150,210,80,0.70)',
    15,     'rgba(240,200,60,0.85)',
    30,     'rgba(240,120,60,0.95)',
    60,     'rgba(200,40,40,1.0)',
  ];
}
